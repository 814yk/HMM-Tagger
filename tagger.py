...
TRITRANSITION_TAG = "tri-trans"
BITRANSITION_TAG = "bi-trans"
UNITRANSITION_TAG = "uni-trans"
EMISSION_TAG = "emit"
...
tri_transition=defaultdict(lambda:defaultdict(lambda: defaultdict(lambda: 1.0)))
bi_transition = defaultdict(lambda: defaultdict(lambda: 1.0))
uni_transition = defaultdict(lambda: 1.0)
...
def viterbi(lines):
    ...
    for (index, line) in enumerate(lines):
            ...
            for q in states:
                ...
                for qq in states:
                    for qqq in states:
                        ...
                        transition_prob = tri_transition[qqq][qq].get(q, None)
                        if not transition_prob:
                            transition_prob = bi_transition[qq][q]
        ...
        for qq in states:
            for q in states:
                transition_prob = tri_transition[qq][q].get(FINAL_STATE, None)
                if not transition_prob:
                    transition_prob = bi_transition[q][FINAL_STATE]
                if transition_prob >= 0:
                    continue

                try:
                    v = V[len(w) - 1][qq][q] + transition_prob
                except KeyError as e:
                    continue

                # Replace if probability is higher
                current = V[len(w)][q].get(FINAL_STATE, None)
                if not current or v > V[len(w)][q][FINAL_STATE]:
                    V[len(w)][q][FINAL_STATE] = v
                    best_final_state = q
                    best_prev_state=qq
        ...
        if best_final_state:
            ...
            for i in range(len(w) - 1, -1, -1):
                ...
                q=best_prev_state
                best_prev_state = back[i][best_prev_state][best_final_state]
                best_final_state=q
...
def main():
    ...
    with open(HMM_FILE, "r") as f:
        for line in f:
            ...
            if line[0] == TRITRANSITION_TAG:
                (qqq,qq, q, trans_prob) = line[1:5]
                tri_transition[qqq][qq][q] = math.log(float(trans_prob))
                states.add(qqq)
                states.add(qq)
                states.add(q)
                
            elif line[0] == BITRANSITION_TAG:
                (qq, q, trans_prob) = line[1:4]
                bi_transition[qq][q] = math.log(float(trans_prob))
                states.add(qq)
                states.add(q)
                '''
            elif line[0] == UNITRANSITION_TAG:
                (q, trans_prob) = line[1:3]
                uni_transition[q] = math.log(float(trans_prob))
                states.add(qq)
                states.add(q)
                '''
